...
impl SqliteStore {
    ...
    fn commit(&self, tx: Transaction<'_>) -> Result<()> {
        tx.commit()?;
    ...
    ///
    /// Drained before the sweep in [`RelayStore::expire`], which is what bounds
    /// the exposure described on [`RelayStore::touch`] to the sweep period.
    fn flush_activity(&self, tx: &Transaction<'_>) -> Result<()> {
        let pending: Vec<(QueueAddress, u64)> = {
            let mut activity = self.lock_activity();
            activity.drain().collect()
        };
        if pending.is_empty() {
            return Ok(());
        }
        ...
        for (recv_addr, at_ms) in pending {
