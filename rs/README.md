...
| [`crates/f2z-intent`](./crates/f2z-intent) | The **cross-app intent bridge**, [`docs/intent-bridge/PROTOCOL.md`](../docs/intent-bridge/PROTOCOL.md) v1 (#905): the versioned request/response format carrying authority delegation from the ZUULI wallet to apps that hold no seed, an unknown version refused before its body is read, the one-use replay ledger, the dual-clock expiry of #528, the caller registry and Android signing-certificate attestation, and the digest that binds an intent to the confirmation the user actually saw. It performs **no cryptography beyond hashing** — the signature, the credential and the transaction all come from other crates; this one decides which bytes they may act on and when. `no_std` + `alloc`, `#![forbid(unsafe_code)]`, no I/O, no clock and no randomness, all three passed in. Ships **no transport**: deep-link delivery is blocked on #461. |
