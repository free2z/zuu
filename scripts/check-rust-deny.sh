...
# Encode an absolute filesystem path for a file:// URL without requiring Python,
# Node, or a newer Bash than macOS ships. Worktrees and TMPDIR may contain spaces
# or URL delimiters even though shell quoting alone makes them safe as paths.
file_url() {
  local input=$1 output= byte encoded i ordinal
  local LC_ALL=C
  i=0
  while ((i < ${#input})); do
    byte=${input:i:1}
    case "$byte" in
      ...
      *)
        printf -v ordinal '%d' "'$byte"
        printf -v encoded '%%%02X' "$((ordinal & 255))"
...
# policies that disagree about that licence. It also judges a generated lockfile
# containing a synthetic yanked crate under warn and deny policies. Passing both
# ways would mean --config is decorative; failing both ways would mean the tree
# is unreachable. A check nobody has watched fail is not a check.
...
EOF
  ...
  # Build a private registry whose only release is yanked. A live crates.io
  # release could be unyanked after this commit and make the self-test lie about
  # a policy change, so the registry state itself is part of the scratch fixture.
  yanked_fixture=$fixture/yanked
  yanked_index=$fixture/yanked-index
  yanked_downloads=$fixture/yanked-downloads
  yanked_name=zuuli-yanked-fixture
  yanked_version=0.0.1
  yanked_source=$fixture/package/$yanked_name-$yanked_version
  mkdir -p "$yanked_fixture/.cargo" "$yanked_fixture/relay/src" \
    "$yanked_index/zu/ul" "$yanked_downloads/$yanked_name/$yanked_version" \
    "$yanked_source/src"

  cat > "$yanked_source/Cargo.toml" <<EOF
[package]
name = "$yanked_name"
version = "$yanked_version"
edition = "2021"
license = "MIT"
EOF
  printf 'pub fn fixture() -> bool { true }\n' > "$yanked_source/src/lib.rs"
  yanked_archive=$yanked_downloads/$yanked_name/$yanked_version/download
  COPYFILE_DISABLE=1 tar -czf "$yanked_archive" -C "$fixture/package" \
    "$yanked_name-$yanked_version"
  yanked_checksum=$(sha256_of "$yanked_archive")
  yanked_downloads_url=$(file_url "$yanked_downloads")
  yanked_index_url=$(file_url "$yanked_index")

  printf '{"dl":"%s/{crate}/{version}/download"}\n' \
    "$yanked_downloads_url" > "$yanked_index/config.json"
  printf '{"name":"%s","vers":"%s","deps":[],"cksum":"%s","features":{},"yanked":true,"links":null}\n' \
    "$yanked_name" "$yanked_version" "$yanked_checksum" \
    > "$yanked_index/zu/ul/$yanked_name"
  git init -q "$yanked_index"
  git -C "$yanked_index" add config.json "zu/ul/$yanked_name"
  git -C "$yanked_index" -c user.name=self-test -c user.email=self-test.invalid \
    commit -q -m 'synthetic yanked registry state'

  cat > "$yanked_fixture/.cargo/config.toml" <<EOF
[registries.self-test]
index = "$yanked_index_url"
EOF
  cat > "$yanked_fixture/relay/Cargo.toml" <<EOF
[package]
name = "zuuli-deny-yanked-consumer"
version = "0.0.0"
edition = "2024"
publish = false

[dependencies]
$yanked_name = { version = "=$yanked_version", registry = "self-test" }

[workspace]
EOF
  printf 'pub fn fixture() -> bool { true }\n' > "$yanked_fixture/relay/src/lib.rs"
  cat > "$yanked_fixture/relay/Cargo.lock" <<EOF
...
name = "$yanked_name"
version = "$yanked_version"
source = "registry+$yanked_index_url"
...
EOF
  if ! yanked_output=$(
    CARGO_HOME="$fixture/cargo-home" "$SELF" --root "$yanked_fixture" \
      --config "$REPO_ROOT/wallet/deny.toml" advisories 2>&1
  ); then
    printf 'self-test FAILED: wallet yanked policy rejected the known-yanked fixture.\n%s\n' \
      "$yanked_output" >&2
    exit 1
  fi
  if ! grep -Fq 'warning[yanked]' <<<"$yanked_output" ||
     ! grep -Fq "$yanked_name $yanked_version" <<<"$yanked_output"; then
    printf 'self-test FAILED: the warn policy passed without reporting the known-yanked crate.\n%s\n' \
      "$yanked_output" >&2
    exit 1
  fi
  printf 'self-test: wallet yanked = "warn" reports a known-yanked crate and exits successfully.\n'

  # Negative control: the same immutable lockfile must fail when only the lint
  # level changes. This proves the successful warn verdict is policy, not a
  # fixture that cargo-deny failed to inspect.
  printf '[advisories]\nyanked = "deny"\n' > "$fixture/yanked-deny.toml"
  if yanked_output=$(
    CARGO_HOME="$fixture/cargo-home" "$SELF" --root "$yanked_fixture" \
      --config "$fixture/yanked-deny.toml" advisories 2>&1
  ); then
    printf 'self-test FAILED: yanked = "deny" accepted the known-yanked fixture.\n%s\n' \
      "$yanked_output" >&2
    exit 1
  fi
  if ! grep -Fq 'error[yanked]' <<<"$yanked_output" ||
     ! grep -Fq "$yanked_name $yanked_version" <<<"$yanked_output"; then
    printf 'self-test FAILED: the deny policy failed for something other than the known-yanked crate.\n%s\n' \
      "$yanked_output" >&2
    exit 1
  fi
  printf 'self-test: yanked = "deny" fails on the same known-yanked crate.\n'

  printf 'self-test: 6 case(s) passed; cargo-deny %s.\n' "$CARGO_DENY_VERSION"
  exit 0
fi
