...
| E2EE messaging | Relay, key-transparency, and MLS services under `rs/` | `wallet/plugins/tauri-plugin-f2zmsg` builds, its two-instance integration test drives two engines over a real relay, and since [#750](https://github.com/free2z/zuu/pull/750) `wallet/zuuli/src-tauri` links it: `Cargo.toml` and its lock carry the plugin, `src/lib.rs` registers it, `src/messaging.rs` serves the enrollment trio, and both capability files grant it | The plugin's own crate gate runs in `zuuli.yml`; the app's gate additionally builds it into ZUULI for desktop, iOS and Android, and compiler-bound IPC probes assert the shipping routers route `plugin:f2zmsg|…` and the unprefixed enrollment commands | **None.** Linking, routing and authorization are proven; no enrollment or message has been performed in a running ZUULI. And first contact still cannot succeed: [#756](https://github.com/free2z/zuu/pull/756) landed the `f2z-kt-client` crate and `directory.rs` now ships a real `KtDirectory` — `/kt/v1/lookup` over HTTPS with §6.3 monotonicity, §8.3's threshold over the client's own witness set, `f2z_kt_core::verify` inclusion proofs, §4.4 re-authorization, and pinning — but the shipping default is still `directory::NoDirectory`, because `KT.md` §12 has not decided the log's identity, the log's signing key, the shipped witness list, or the default *t*, and `f2z_kt_core::WitnessSet` deliberately has no `Default`: a plugin that invented one would be inventing the MITM answer on everyone's behalf. So `resolve_handle`, `start_conversation` and `accept_contact_request` still fail closed with `witness-threshold-unmet` — the required behaviour under §6.4, not a placeholder. Even a configured directory could not complete `start_conversation`, since `KT.md` §4.1 publishes no `KeyPackage` | **Linked and reachable; still not usable.** A tester now receives the Messages surface and can reach enrollment, and **cannot start a conversation with anyone.** The remaining blocker is an undecided operator configuration — not a missing key-transparency client, and not the dependency graph. [#753](https://github.com/free2z/zuu/issues/753) is closed: since [#759](https://github.com/free2z/zuu/pull/759) a messaging store that will not open no longer takes ZUULI down at launch — the app starts, `get_engine_status` reports `state: "faulted"` with `lastError`, pure `check_handle_eligibility` still answers and rejects malformed handles, and every engine- or storage-dependent plugin command plus the enrollment trio refuses with the same §8 code instead of panicking, while `SqliteBackend::open`'s strictness is unchanged ([#762](https://github.com/free2z/zuu/issues/762)). Epic: [#305](https://github.com/free2z/zuu/issues/305). Do not describe ZUULI as having messaging because the plugin is linked. |
