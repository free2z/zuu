...
export function verifyMacosKeychainEntitlements(tauriSource, entitlementsSource) {
  const failures = [];
  let tauri;
  try {
  ...
  if (entitlementsSource !== CANONICAL_ENTITLEMENTS) {
    failures.push(
      "macOS entitlements must be the reviewed application identifier, team identifier, and exact Keychain access group",
    );
  }
  return failures;
}
...
async function main() {
  ...
  console.log("macOS Keychain entitlements are source-bound and complete.");
}
