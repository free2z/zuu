      ...
      <nav className="hidden md:flex flex-col w-52 bg-zuuli-surface border-r border-zinc-800/50 py-6 gap-0.5 shrink-0" aria-label="Main navigation">
        ...
        {walletStatus?.activeWalletName && (
          <button
            ...
            className="flex items-center gap-2 px-6 py-2.5 mb-6 text-xs text-zinc-500 hover:text-purple-400 transition-colors group min-tap"
            aria-label={`Switch wallet, current: ${walletStatus.activeWalletName}`}
          >
            {walletIcon}
            <span className="truncate">{walletStatus.activeWalletName}</span>
            {(walletStatus.walletCount ?? 0) > 1 && (
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="shrink-0 opacity-50 group-hover:opacity-100" aria-hidden="true">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-zuuli-surface/95 backdrop-blur-sm border-t border-zinc-800/50 flex z-50" aria-label="Main navigation">
        {navItems.map(({ page, label, icon }) => (
          <button
            ...
            aria-current={currentPage === page ? "page" : undefined}
            aria-label={label}
            className={`flex-1 flex flex-col items-center gap-1 py-3 text-[10px] transition-colors min-tap ${
              currentPage === page
                ? "text-purple-400"
                : "text-zinc-500"
            }`}
