/**
 * The English word list used by the shipping bip0039 0.12 parser. English is
 * the crate's only default-enabled language; keep this list aligned when that
 * native dependency or its language features change.
 */
...
export const BIP39_ENGLISH_WORD_INDEX = new Map<string, number>(
  BIP39_ENGLISH_WORDS.map((word, index) => [word, index] as const),
);
