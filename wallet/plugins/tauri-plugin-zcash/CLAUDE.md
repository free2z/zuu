...
- **Seed custody** — all seed operations go through `WalletState::{store,get,delete}_seed_phrase`, which use `spawn_blocking` because native APIs are synchronous and may prompt. New data is native-only: macOS/iOS Keychain, AndroidKeyStore-backed AES-GCM, persistent Linux Secret Service, or Windows Credential Manager. `.seeds/*.enc` is read-only migration input. Only `SecureStoreError::NotFound` may enter migration; cancellation, lock, corruption, and backend failures fail closed. Migration must validate the seed-derived UFVK, native-write, native-readback, and only then delete legacy material.
