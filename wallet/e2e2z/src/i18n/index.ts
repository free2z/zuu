...
function readSavedLocale(): unknown {
  try {
    ...
    // Storage may be unavailable in a hardened webview. The browser locale
    // remains a deterministic fallback and no setting is mutated here.
    return null;
  }
}

export interface InitializeAppI18nOptions {
  savedLocale?: unknown;
...
export async function initializeAppI18n(
  ...
  const savedLocale = Object.prototype.hasOwnProperty.call(options, "savedLocale")
    ? options.savedLocale
    : readSavedLocale();
  ...
  const { catalogLocale } = resolveLocaleSelection({ savedLocale, browserLocale });
  const documentElement =
    options.documentElement ?? globalThis.document?.documentElement;
  if (documentElement) documentElement.lang = catalogLocale;
  return createAppI18n(catalogLocale ?? DEFAULT_LOCALE);
}
