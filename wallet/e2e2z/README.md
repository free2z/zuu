...
`src-tauri/gen/android` and are **committed**, the way ZUULI's are. They are
generated by `tauri ios init` / `tauri android init`, but the tree carries a
handful of reviewed corrections on top of the generator's output; see
`scripts/release-identity.mjs`, which asserts each of them and fails the build
if one is lost.

The pipeline is `.github/workflows/e2e2z-release.yml`, mirroring
...
by a separate job in the protected `e2e2z-app-stores` environment, and verified
again afterwards. The workflow's header comment explains why this app gets its
own environment rather than sharing ZUULI's.
...
The manifest declares exactly one: `INTERNET`. e2e2z has no camera, no
microphone and no wallet, so none of ZUULI's other permissions apply, and
`release-identity.mjs` fails the build if a second one appears. The merged
manifest that ships also carries
`cash.free2z.e2e2z.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION`, a signature-level
self-permission `androidx.core` injects; the release workflow asserts that set
exactly, so a third entry is a red build.

Backups are off: `allowBackup=false` plus `backup_rules.xml` and
`data_extraction_rules.xml` excluding every domain, so device keys and the
message store cannot leave the device through a cloud backup or a
...
are not changes — trailing whitespace from the deep-link plugin, quoting and
trailing newlines from xcodegen — and the release proves the committed project
is the one that was built with `git diff --exit-code`.
