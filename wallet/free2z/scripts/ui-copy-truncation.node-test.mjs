 ...
 *
 * The runtime counterpart lives in `tests/ui-copy.pw.ts`, which catches
...
test("product chrome contains no Search implementation explainer", () => {
  ...
  // ZUULI anchors this on its Search surface, where the reported copy lived.
  // That feature has not moved here yet (#904), so the anchor is this app's
  // article Feed — the screen that carries the search field and the article
  // pitch the rules below are written against. The point of the anchor is
  // unchanged: prove the scan actually reached a real chrome surface rather
  // than passing on an empty selection.
  assert.ok(
    CHROME_FILES.some(
      (file) => relative(ROOT, file) === "src/features/articles/pages/Feed.tsx",
    ),
    "product chrome source scan omitted the reported article Feed surface",
  );
