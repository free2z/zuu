#!/usr/bin/env bash
# Build, sign and optionally publish the free2z Android bundle from a workstation.
#
# This is the operator path. The workflow in .github/workflows/free2z-release.yml
...
#
# Mirrors wallet/e2e2z/scripts/mobile-release.sh, which mirrors ZUULI's. Like
# e2e2z's it keeps the store-identity preflight, which ZUULI does not need
# because ZUULI's Play listing already exists. free2z's does not, so today this
# script is expected to stop in its first command; that is the point.
set -euo pipefail
umask 077
...
if [[ -n "${FREE2Z_RELEASE_SOURCE_SHA:-}" ]]; then
  identity_args+=("--source-sha=$FREE2Z_RELEASE_SOURCE_SHA")
fi
...
if [[ "$mode" == --upload ]]; then
  if [[ ! "${FREE2Z_RELEASE_SOURCE_SHA:-}" =~ ^[0-9a-f]{40}$ ]]; then
    echo "refusing store upload: FREE2Z_RELEASE_SOURCE_SHA must be the exact full source SHA" >&2
    exit 65
  fi
  if [[ "${FREE2Z_CONFIRM_UPLOAD:-}" != "$identity" ]]; then
    echo "refusing store upload: set FREE2Z_CONFIRM_UPLOAD=$identity" >&2
    exit 65
  fi
fi
...
cp "$aab" "release-artifacts/free2z-${identity}-android.aab"
...
echo "free2z $identity Android release validation completed ($([[ "$mode" == --upload ]] && echo uploaded || echo dry-run))."
