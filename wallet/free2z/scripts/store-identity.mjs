#!/usr/bin/env node
//
// Read `wallet/free2z/store-identity.json` and refuse, in words a human can act
// on, to proceed past a store record that does not exist yet.
//
// Why this file exists. Everything else in the free2z release path mirrors
// e2e2z's, which mirrors ZUULI's, and ZUULI hardcodes its two store identities
// directly in the workflow: the provisioning profile `ZUULI App Store CI`, its
// UUID, and the Android upload certificate fingerprint. It can, because those
// things exist. For `cash.free2z.free2z` the two sides are at different stages,
...
// file, start the unknown ones at `null`, and make every step that needs one
// fail closed with the sentence that says what to go and do. A pipeline that
// stops at a stated blocker is the goal. A pipeline that appears to work and
// does not is the failure this file exists to prevent.
//
...
const APPLE_RECORD_BLOCKER = [
  "cash.free2z.free2z has no App Store Connect app record.",
  "",
  ...
  "     for cash.free2z.free2z.",
  "  2. App Store Connect -> Apps -> New App: create the app record on that",
  "     bundle id.",
  "  3. Profiles -> new App Store distribution profile for cash.free2z.free2z,",
  "     signed by the existing Corpora Apple Distribution certificate.",
  "  4. Record that profile's name and UUID in wallet/free2z/store-identity.json",
...
const PLAY_LISTING_BLOCKER = [
  "cash.free2z.free2z has no Google Play Console listing.",
  "",
  ...
  "account must:",
  "",
  "  1. Play Console -> Create app, package name cash.free2z.free2z, and complete",
  "     enough of the dashboard for an internal-testing release to be accepted.",
  "     free2z declares CAMERA and RECORD_AUDIO, so the Data safety form and the",
  "     permissions declaration must describe live-stream capture; that is a",
  "     listing task, not a release task, and it is why this blocker is a human",
  "     step rather than an API call.",
  ...
  "     wallet/free2z/store-identity.json (uppercase, colon-delimited, exactly as",
  "     `keytool -list -v` prints it), with the keystore itself stored as",
  "     ANDROID_KEYSTORE_BASE64 in the free2z-app-stores environment.",
  ...
  "google.appSigningCertificateSha256. That value is not used by this release",
  "path.",
].join("\n");
...
export function validate(identity) {
  ...
  if (identity.applicationId !== "cash.free2z.free2z")
    fail(`store-identity.json applicationId must be cash.free2z.free2z, got ${JSON.stringify(identity.applicationId)}`);
  ...
  if (apple === null || typeof apple !== "object" || Array.isArray(apple)) {
    ...
    // A record with no profile is a real intermediate state — it is exactly the
    // state free2z is in today; a profile named against a record that does not
    // exist is not, and neither is half a profile. Both would sail through the
...
export function appleBlockers(identity) {
  ...
  if (identity.apple?.appStoreConnectRecord !== "created")
    ...
    blockers.push(APPLE_PROFILE_BLOCKER);
  return blockers;
}
...
export function androidBlockers(identity) {
  ...
  if (identity.google?.playConsoleListing !== "created")
    ...
    blockers.push(
      "cash.free2z.free2z has a Play Console listing but no recorded upload " +
        "certificate fingerprint. Record google.uploadCertificateSha256 in " +
        "wallet/free2z/store-identity.json; signing refuses to run without a " +
...
function selfTest() {
  ...
  expect("foreign application id", validate(mutated), "applicationId must be cash.free2z.free2z");
  ...
  expect("malformed profile UUID", validate(mutated), "must be a lowercase UUID or null");
