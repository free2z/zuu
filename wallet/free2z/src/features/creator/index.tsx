...
function CreatorProfile({
  ...
  // The live marker must only appear when this creator is ACTUALLY live right
  // now. Gate it on the server-computed `is_live` from the creator payload so
  // it renders instantly with NO probe on mount. See `useLiveGate` for the
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="min-w-0 pb-1">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
              ...
              {/* `can_stream` is an INTERNAL streaming-eligibility flag we track
                  ...
                  the live-status probe on the live marker below. */}
            </div>
          </div>
        </div>
        ...
        >
          {/* ZUULI links this straight to `/live/{username}`. `features/live`
              has not moved to this surface yet (#904), so there is no room to
              send a fan to and a 404 is worse than no destination. The
              live-status probe is unchanged and still tells the reader the
              creator is on air right now; only the navigation waits for the
              feature. */}
          {isLive ? (
            <Badge variant="live" data-creator-live>
              <Radio className="me-1.5 h-3.5 w-3.5" aria-hidden />
              Live now
            </Badge>
          ) : null}
...
function BackLink() {
  ...
  // ZUULI falls back to Search here. That feature has not moved to this surface
  // yet (#904), so the fallback is Articles — the only other destination a
  // reader can have arrived from.
  return (
    <Link
      to="/articles"
      className="min-tap inline-flex items-center gap-2 rounded-md text-sm text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
