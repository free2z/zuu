...
describe("Markdown links", () => {
  ...
  it("keeps a route this surface actually mounts routed as-is", () => {
    const markup = render("[top up](/fund)", "article");

    expect(markup).toContain('href="/fund"');
  });

  // ZUULI owns /wallet, /messages and /kyc; this app owns none of them, so a
  // link to one is creator content rather than an in-app route (see
  // `@/lib/routes`). Rewriting it to the article route is the correct read of
  // an authored free2z path, not a lost destination.
  it("treats a ZUULI-only path as free2z content rather than an app route", () => {
    const markup = render("[wallet](/wallet/send)", "article");

    expect(markup).not.toContain('href="/wallet/send"');
    expect(markup).toContain('href="/articles/send"');
  });
