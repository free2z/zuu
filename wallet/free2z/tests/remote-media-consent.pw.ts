...
test("a card consent click loads only its cover and never navigates", async ({
  ...
  const observed = await observeRemoteMedia(page);
  // ZUULI reaches the card through Search. That feature has not moved to this
  // surface yet (#904); the article Feed renders the same `ArticleCard`, so the
  // card-level consent boundary is exercised at the same component. The fixture
  // is deliberately old enough not to reach the unfiltered feed, so the tag
  // filter is what selects it.
  await page.goto("/articles?tags=privacy,test");
