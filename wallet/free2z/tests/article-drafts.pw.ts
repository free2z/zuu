...
test("a pending local draft survives in-app navigation and reload", async ({ page }) => {
  ...
  // ZUULI leaves the composer via Home. This surface has no Discover home —
  // Articles is its front door — so the same in-app departure is the Articles
  // destination, which is what forces the synchronous persistence boundary.
...
test("publish success preserves a newer revision saved by another tab", async ({
  ...
  await newerTab
    ...
    .first()
    .click();
  await expect(newerTab).toHaveURL(/\/articles$/);
