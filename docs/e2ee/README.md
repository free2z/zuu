...
| [`decisions/`](./decisions/) | One ADR per decision, `0001`–`0017`. |
...
| [0009](./decisions/0009-queue-addressing-and-binding.md) | **Queue addressing** — the recipient creates the queue and the **relay** generates both addresses from its own CSPRNG; `send_addr` is advertised in-band inside MLS; `BIND_SEND` is **once-only and irreversible**. Anyone who learns the address and binds first takes the write capability — the design makes that **noisy, not impossible**, and identifies the relay returning the refusal rather than the actor. Narrows `ARCHITECTURE.md` §5.4. |
...
| [0016](./decisions/0016-enrollment-sealing-boundary.md) | **Enrollment sealing** — the app that generated a device's secrets seals them, under a per-device `DeviceWrapKey` it samples and holds; `IdentityInstall.wrap_key` — today the seed-derived `BackupWrapKey` — is **removed**, so no seed-derived key ever reaches `cash.free2z.e2e2z`. `IssueDeviceCredentialResultV1` gains **no fields**: `identity_pk` and `handle` are already inside the signed `DeviceCredential`, and a second unsigned copy beside it is one an unauthenticated responder would get to choose ([#929](https://github.com/free2z/zuu/issues/929)) — but the caller **MUST** still refuse a credential whose handle is not the one *it requested*, which is a value the caller already holds rather than one the responder supplies, and is the only authenticity check trust-on-first-response permits. The install step is an e2e2z **app-crate** command needing no capability and no `zcash:*` grant. `device_kem_pk`'s circularity is **deliberately left open**, so enrollment stays incomplete even after a transport lands. |
| [0017](./decisions/0017-internal-directory-activation.md) | **Internal directory activation**: a **disposable internal** key-transparency log. free2z runs the log and its only witness, *t* = 1, and the witness is **not** independent, so lookups resolve while the warning stays up. The values come from `internal-directory.conf`, which is compiled in and **fails closed** while it holds placeholders. ZUULI signs and submits the `DirectoryEntry` with a backend-issued `HandleAssertion` (Contract C, whose bytes are ratified here with shared vectors), and the log refuses any other authority. `mergedAtEpoch` comes only from a verified lookup of the device's own handle, never from a receipt. e2e2z publication needs one request field, argued here against ADR 0016 §4. §8 lists what must change before public launch. |
