...
Every plugin command in §3 must appear in four places, and a mismatch is a build
failure by the same mechanism `scripts/check-zcash-permissions.mjs` already
enforces for the Zcash plugin:
...
nobody updates §3, and the contract quietly becomes fiction. That is exactly the
drift `scripts/check-zcash-permissions.mjs` already prevents between
`build.rs`'s `COMMANDS`, `invoke_handler`, `permissions/` and the capability
files.

A companion `scripts/check-client-contract.mjs` should extend the same
mechanical check to this document — parsing §3's command tables and asserting
they match `tauri-plugin-f2zmsg`'s `COMMANDS` exactly, in both directions — once
that plugin exists. It is proposed rather than written here, because a checker
with nothing to check is a checker that gets deleted.
